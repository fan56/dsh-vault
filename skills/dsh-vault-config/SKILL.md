...
description: "dsh 加密备份插件（@aiwayds/dsh-vault）使用与配置指南。凡涉及 dsh 配置备份、跨机器迁移、/vault backup/restore/list，或要配置 vault 时先读本指南：profile patch（cordis.patch.yml）里 `dsh-vault` 条目的 config 段（repo/machineDescription/rememberPassphrase）、首次备份 ask_user_question 向导（收集仓库/机器描述/口令记忆后代写配置）、口令三种来源（参数/env/钥匙串）、GitHub 凭据（GITHUB_TOKEN 或 gh 登录）、口令遗失不可解。触发词：vault、备份、恢复、迁移、快照、钥匙串、passphrase、dsh-backup。"
---
...
## 配置入口（profile patch 的 `dsh-vault` 段）

dsh 0.1.7 起 settings.yaml 只在启动时导入一次即改名 `settings.yaml.imported`，
插件配置改存 profile patch（`~/.dsh/profiles/<profile>/cordis.patch.yml`）：
...
- 三个键都可用 `/vault set repo|machine-desc|remember-passphrase` 运行时写回（即时校验、
  免重启生效）；手工编辑 profile patch 同样有效（改 config 触发插件重挂）。
- 旧 settings.yaml 顶层 `vault:` 段**不会**自动迁到 `dsh-vault` 条目——升级后该段原样
  留在 `settings.yaml.imported`，用 `/vault set` 重新设置一次即可。
...
收集完代写进 profile patch 的 `dsh-vault` 条目 config 段（或引导用户直接 `/vault set`），
然后引导首次备份，注意两条红线：
...
1. **备份集边界**：settings.yaml、settings.yaml.imported、`.credentials.yaml`、
   APPEND_SYSTEM.md、`agents/`、各 profile 清单四件套（含 cordis.patch.yml）、
   models-store.json **进**备份；sessions/、storages/、
...
4. **卸载遗留**（`dsh plugin remove` 都不会碰，需手动处理）：钥匙串 `dsh-vault` 条目、
   GitHub 私有仓 `dsh-backup-<用户名>`、`~/.dsh/vault/stash/`、profile patch 里
   `dsh-vault` 条目的 config 段、`settings.yaml.imported` 里的旧 `vault:` 段存档。
